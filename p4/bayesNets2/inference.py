...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        joinableFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)


        """iterate through all variables in order"""
        for variable in eliminationOrder:

            """join factors that can be joined by current in order variable"""
            joinableFactors, joinedFactor = joinFactorsByVariable(joinableFactors, variable)
        ...
        joinedFactor = joinFactors(joinableFactors)
        jointNormalizedFactor = normalize(joinedFactor)

        return jointNormalizedFactor
